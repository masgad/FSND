...
class Show(db.Model):
	...
	artists = db.relationship("Artist", backref=db.backref("shows", cascade="all, delete-orphan"))
	venues = db.relationship("Venue", backref=db.backref("shows", cascade="all, delete-orphan"))
...
def venues():
  ...
  #db.session.query(Show.venue_id, func.count(*)).group_by(Show.venue_id).filter(Show.start_time < datetime.now()).all()
  
  print("UPCOMING SHOWS:", upcoming_shows)
  print(type(data))
  #print(upcoming_shows.venue_id, upcoming_shows.count)
...
def shows():
  ...
  #   "start_time": "2035-04-15T20:00:00.000Z"
  # }]
  data = db.session.query(Artist, Show, Venue).join(Show, Show.artist_id == Artist.id and Show.venue_id == Venue.id).distinct(Show.id)
  #print(data[0].Show.start_time)
   #print ("Artist: {} Name: {}".format(x.Department.name, x.Employee.name))
